...
local sql = setmetatable({},{
	__call = function(self)
		...
		self.insert = function(plate,...)
			...
			return MySQL.insert.await(str:format('bubba_stancer','plate','setting'),{plate,...})
		end
		...
		self.save = function(string, data)
			...
			if isExist[1] then
				return MySQL.update(str:format('bubba_stancer','plate'),{data,string})
			else
		...
		self.query = function(string)
			...
			return MySQL.query.await(str:format('bubba_stancer','plate'),{string})
		end

        self.fetch = function()
            return MySQL.query.await('SELECT * FROM bubba_stancer')
        end
...
Citizen.CreateThreadNow(function()
	local success, result = pcall(MySQL.scalar.await, 'SELECT 1 FROM bubba_stancer')

	if not success then
		MySQL.query.await([[CREATE TABLE `bubba_stancer` (
...
RegisterNetEvent('bubba_stancer:servercallback', function(name,...)
    local source = source
    TriggerClientEvent('bubba_stancer:servercallback',source,name,callbacks[name](source,...))
end)
...
RegisterNetEvent("bubba_stancer:save")
AddEventHandler("bubba_stancer:save", function(stance)
...
RegisterNetEvent("bubba_stancer:addstancer") -- Standalone Purpose
AddEventHandler("bubba_stancer:addstancer",function(vehicle) 
