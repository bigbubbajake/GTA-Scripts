...
RegisterNetEvent("bubba_stancer:openstancer")
AddEventHandler("bubba_stancer:openstancer", function(vehicle,val,coords)
	OpenStancer()
end)
...
callback = function(name,...)
	...
	TriggerServerEvent('bubba_stancer:servercallback',name,...)
	return Citizen.Await(callbacks[name])
end

RegisterNetEvent('bubba_stancer:servercallback', function(name,data)
...
RegisterNUICallback('wheelsetting', function(data, cb)
    ...
    if vehicle ~= nil and vehicle ~= 0 then
		...
		TriggerServerEvent('bubba_stancer:save',veh_stats[plate])
...
function OpenStancer()
	...
	if Config.Framework == 'Standalone' and not ent.stancer then
		TriggerServerEvent('bubba_stancer:addstancer')
