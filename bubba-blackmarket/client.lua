...
RegisterNetEvent('bubba-blackmarket:client:CreatePed', function ()
    QBCore.Functions.TriggerCallback('bubba-blackmarket:server:PedLocation', function(data)
        local bm = data.bm
        loc = data.loc
        ...
        exports['qb-target']:AddCircleZone("['"..bm.."("..loc..")']", vector3(sdata["coords"][loc].x, sdata["coords"][loc].y, sdata["coords"][loc].z), 2.0, { name="['"..bm.."("..loc..")']", debugPoly=Config.Debug, useZ=true, },{ options = { { event = "bubba-blackmarket:ShopMenu", icon = "fas fa-certificate", label = Lang:t("target.browse"),item = (sdata.openwith or nil),gang = (sdata.gang or nil), shoptable = sdata, products = productstable, name = sdata["label"], k = bm, l = loc, }, }, distance = 2.0 })
    end)
end)
...
local function mainthread()
    for k, v in pairs(Config.Locations) do
        ...
        if Config.RandomLocation then
            Wait(500)
            TriggerEvent('bubba-blackmarket:client:CreatePed')
        else
            for l, b in pairs(v["coords"]) do -- Create ped for each location given in Config
                ...
                exports['qb-target']:AddCircleZone("Shop - ['"..k.."("..l..")']", vector3(b.x, b.y, b.z), 2.0, { name="Shop - ['"..k.."("..l..")']", debugPoly=Config.Debug, useZ=true, },{ options = { { event = "bubba-blackmarket:ShopMenu", icon = "fas fa-certificate", label = Lang:t("target.browse"), item = (v.openwith or nil),gang = (v.gang or nil), shoptable = v, products = productstable, name = v["label"], k = k, l = l, }, }, distance = 2.0 })
            end
        end
    end
end

RegisterNetEvent('bubba-blackmarket:ShopMenu', function(data, custom)
    ...
    if rmi then
        TriggerServerEvent('bubba-blackmarket:server:AddRemoveItem', 'remove')
        inmenu = true
    end

    ShopMenu[#ShopMenu + 1] = { header = data.shoptable["label"], txt = "", isMenuHeader = true }
    ShopMenu[#ShopMenu + 1] = { header = "", txt = Lang:t("menu.close"), params = { event = "bubba-blackmarket:CloseMenu" } }
    ...
    for i = 1, #products do
            ...
            for i2 = 1, #products[i].requiredJob do
                if QBCore.Functions.GetPlayerData().job.name == products[i].requiredJob[i2] then
                    ShopMenu[#ShopMenu + 1] = { icon = products[i].name, header = setheader, txt = text, isMenuHeader = lock,
                        params = { event = "bubba-blackmarket:Charge", args = { item = products[i].name, cost = totalprice, info = products[i].info, shoptable = data.shoptable, k = data.k, l = data.l, amount = amount, custom = custom, rem = rmi} } }
                end
            end
        elseif products[i].requiresLicense then
            ...
            ShopMenu[#ShopMenu + 1] = { icon = products[i].name, header = setheader, txt = text, isMenuHeader = lock,
                    params = { event = "bubba-blackmarket:Charge", args = { item = products[i].name, cost = totalprice, info = products[i].info, shoptable = products, k = data.k, l = data.l, amount = amount, custom = custom, rem = rmi} } }
        end
        text, setheader = nil
    end
...
AddEventHandler('qb-menu:client:closeMenu', function()
    if rmi and inmenu then
        TriggerServerEvent('bubba-blackmarket:server:AddRemoveItem', 'add')
        inmenu = false
    end
end)

RegisterNetEvent('bubba-blackmarket:CloseMenu', function() exports['qb-menu']:closeMenu() end)

RegisterNetEvent('bubba-blackmarket:Charge', function(data)
    ...
    if dialog then
        ...
        if tonumber(dialog.amount) <= 0 then TriggerEvent("QBCore:Notify", Lang:t("error.incorrect_amount"), "error") TriggerEvent("bubba-blackmarket:Charge", data) return end
        if data.cost == "Free" then data.cost = 0 end
        TriggerServerEvent('bubba-blackmarket:GetItem', dialog.amount, dialog.billtype, data.item, data.shoptable, data.cost, data.rem)
...
if Config.EnableHacking then
                ...
                action = function()
                    TriggerEvent('Mx::StartMinigameElectricCircuit', "50%", "50%", "1.0", "30vmin", "dial.ogg", function()
                        QBCore.Functions.Progressbar('dial_connect', 'Tapping Conenctions..', 21000, false, false, {
                            ...
                            TriggerEvent("bubba-blackmarket:client:HackSuccess")
                ...
                action = function()
                    TriggerEvent('Mx::StartMinigameElectricCircuit', "50%", "50%", "1.0", "30vmin", "dial.ogg", function()
                        QBCore.Functions.Progressbar('dial_connect', 'Tapping Conenctions..', 21000, false, false, {
                            ...
                            TriggerEvent("bubba-moneywash:client:HackSuccess")
                        end, function() -- Play When Cancel
                            --Stuff goes here
                        end)
                    end)
    ...
    local function FinishMinigame(faults)
        if faults > 0 then
            ...
            TriggerEvent("bubba-blackmarket:client:GetLocation")
        end
    end

    RegisterNetEvent("bubba-blackmarket:client:CreateBMBlip", function(coords)
        local transG = 250
    ...
    RegisterNetEvent("bubba-blackmarket:client:HackSuccess", function() 
    ...
    RegisterNetEvent("bubba-blackmarket:client:GetLocation", function() 
        QBCore.Functions.Notify('You will get the location now', 'success', 7500)
        if Config.RandomLocation then
            QBCore.Functions.TriggerCallback('bubba-blackmarket:server:GetBMLocation', function(data)
                QBCore.Debug(data)
                local loc = Config.Locations[data.bm].coords[data.loc]
                local locv3 = vector3(loc.x, loc.y, loc.z)
                print(locv3)
                TriggerEvent("bubba-blackmarket:client:CreateBMBlip", locv3)
            end)
        else
            local bmlocnum = 0
            for k, v in pairs(Config.Locations) do
    ...
    RegisterNetEvent("bubba-blackmarket:client:wiretappingCall", function(coords)
...
RegisterNetEvent("bubba-blackmarket:client:removeall",function()
    for k, v in pairs(Config.Locations) do
        if Config.RandomLocation then
...
AddEventHandler('onResourceStop', function(resource) if resource ~= GetCurrentResourceName() then return end
    TriggerEvent("bubba-blackmarket:client:removeall")
end)
