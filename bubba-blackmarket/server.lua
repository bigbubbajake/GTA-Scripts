...
RegisterNetEvent('bubba-blackmarket:server:RandomLocation', function()
...
if Config.UseTimer then
    CreateThread(function()
        ...
        while true do
            ...
            minutes = minutes - 1
            if minutes == 0 then
                TriggerEvent('bubba-blackmarket:server:RandomLocation')
                TriggerClientEvent("bubba-blackmarket:client:removeall", -1)
                TriggerClientEvent('bubba-blackmarket:client:CreatePed', -1)
...
QBCore.Functions.CreateCallback("bubba-blackmarket:server:PedLocation", function (_, cb)
    cb(location)
end)

QBCore.Functions.CreateCallback("bubba-blackmarket:server:GetBMLocation", function (_, cb)
    cb(location)
end)
...
RegisterServerEvent('bubba-blackmarket:GetItem', function(amount, billtype, item, shoptable, price, removeitem)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local BlackMoneyName = Config.BlackMoneyName
    ...
    if removeitem == nil and not Config.RemoveItem then
        TriggerClientEvent('bubba-blackmarket:ShopMenu', src, data, custom)
...
RegisterNetEvent('bubba-blackmarket:server:AddRemoveItem', function (action)
    local Player = QBCore.Functions.GetPlayer(source)
    if action == 'add' then
        Player.Functions.AddItem(Config.ItemName, 1)
    end
...
RegisterNetEvent("bubba-blackmarket:server:callCops", function(coords)
    local alertData = {
        title = "10-33 | Shop Robbery",
        coords = {x = coords.x, y = coords.y, z = coords.z},
        description = "Someone Is WireTapping Phonecalls!"
    }
    TriggerClientEvent("bubba-blackmarket:client:wiretappingCall", -1, coords)
